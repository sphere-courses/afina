...
void *Worker::RunOnRunProxy(void *proxy_args) noexcept {
    ...
    try {
        ...
        args->worker_->OnRun(args->socket_, args->fifo_read_fd_, args->fifo_write_fd_);
        delete args;
...
// See Worker.h
void Worker::Start(int server_socket, int fifo_read_fd, int fifo_write_fd) throw() {
    std::cout << "network debug: " << __PRETTY_FUNCTION__ << std::endl;
    running_.store(true);

    auto args = new Worker::ProxyArgs{
            .worker_ = this, .socket_ = server_socket,
            .fifo_read_fd_ = fifo_read_fd, .fifo_write_fd_ = fifo_write_fd};
    pthread_t buffer;
...
// See Worker.h
void Worker::OnRun(int server_socket, int fifo_read_fd, int fifo_write_fd) noexcept {
    std::cout << "network debug: " << __PRETTY_FUNCTION__ << std::endl;
    ...
    try {
        manager_ = new EpollManager(server_socket, fifo_read_fd, fifo_write_fd, this);
    } catch (std::exception &exception) {
